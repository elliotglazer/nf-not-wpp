...
module FormalizationTemplate
  # Retain the starter template's guard and also fail closed on this
  # repository's explicit user-owned publication placeholders.
  # Publication placeholders are invalid wherever they occur in a scalar;
  # otherwise a prose prefix could hide an unresolved legal or authorship TODO.
  SENTINEL = /(?:TEMPLATE(?::|\z)|TODO(?:\(|:|\z))/
  ...
  def self.validate(path, expect_template: false)
    ...
    raise ValidationError, <<~MESSAGE.chomp
      #{path} still contains #{placeholders.length} unresolved placeholder value(s):
      #{locations}
  ...
  def self.run_cli(arguments, output: $stdout, errors: $stderr)
    ...
    if expect_template
      ...
      output.puts "#{path} contains no unresolved placeholder values"
    end
    0
