...
packages to full immutable SHAs. The exact 25-file Flypitch4 source snapshot is
vendored under [`vendor/flypitch4/`](vendor/flypitch4/README.md) with its
Apache-2.0 licence; `SOURCES.sha256` fixes every retained source byte and is
checked by CI. The selected Lean version is supported by the matching pinned
`lean4export` revision used by the verification script.
