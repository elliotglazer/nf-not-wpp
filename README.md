...
The Lake package currently pins or vendors:

| Input | Revision |
| --- | --- |
| Lean | `leanprover/lean4:v4.30.0` |
| Mathlib | `c5ea00351c28e24afc9f0f84379aa41082b1188f` |
| Vendored Flypitch4 sources | `503dd00ba677b42628a878ad263cc116abb0a8f7` |

`lake-manifest.json` was generated by Lake and resolves all nine external Git
packages to full immutable SHAs. The 25-file Flypitch4 source snapshot is
vendored under [`vendor/flypitch4/`](vendor/flypitch4/README.md) with its
Apache-2.0 licence. Twenty-one files remain byte-for-byte upstream copies; four
carry the documented, semantics-preserving Lean 4.30 compatibility edits in
[`COMPATIBILITY.md`](vendor/flypitch4/COMPATIBILITY.md). `SOURCES.sha256` fixes
every retained source byte and is checked by CI. The selected Lean version is
supported by the matching pinned `lean4export` revision used by the
verification script.
...
The ordinary Solution build compiles the eight-module Flypitch4 closure rooted
at `Flypitch4.Completeness`. For an optional audit of the complete vendored
library, run `LEAN_NUM_THREADS=1 lake build Flypitch4`.
...
The setup/Challenge stages, vendored Flypitch build, standalone Solution,
