...
This is a source-only Palomar packaging candidate for a Lean 4 proof that the
first-order theory generated by the eleven literal Hailperin axioms for New
...
inside `provenance/metamath/accepted-translator-input.mm`. The complete
19-stage repair account, translator/profile hashes, exact accepted input, C18
receipts, and evidence boundary are retained in
`provenance/METAMATH_PROVENANCE.md` and `provenance/provenance.json`. The exact
...
The release repository must not depend on absolute local paths, local package
overrides, checked-in build products, credentials, submodules, or unresolved
path-order selection. See `PALOMAR_CHECKLIST.md` for the evidence gates.
