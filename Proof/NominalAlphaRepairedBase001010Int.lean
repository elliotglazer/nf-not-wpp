...
noncomputable def nominal_df_int
    (x : Var) (y : Var) (A : Class) (dv_A_x : x ∉ A.fv) (dv_A_y : y ∉ A.fv) (dv_x_y : x ≠ y) :
    Nominal.NPrf (.classEq (syn_cint A) (.cab x (.all y (.imp (.classMem (.cv y) A) (.objMem x y))))) := by
  ...
  exact Nominal.alphaClassEq (TAlphaClass.cab (TAlphaWff.all (TAlphaWff.imp (TAlphaWff.classMem (TAlphaClass.cv (TAlphaVar.here _ _ _)) (TAlphaClass.refl_of_fv_fresh _ _ (by simp_all [TEnvFresh] <;> aesop (config := { maxRuleApplications := 2000 })))) (TAlphaWff.objMem (TAlphaVar.there (freshVar_injective ((A).fv) (by decide)) dv_x_y (TAlphaVar.here _ _ _)) (TAlphaVar.here _ _ _)))))
